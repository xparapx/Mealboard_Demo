...
MEAL = {"fetched_at": "2026-08-24T05:40:00", "week": [
    {"date": "20260824", "menu": ["현미밥", "김치찌개 (1.5.6.10.13)", "*우유 (2)"], "kcal": 1094.1,
     ...
     "dinner": {"menu": ["잡곡밥", "된장국 (5.6)"], "kcal": 802.0,
                "assess": {"energy_pct": 89, "macro_ratio": {"carb": 60, "protein": 14, "fat": 26}, "macro_ratio_ok": True, "mar": 88},
                "carbon": {"kgco2e": 1.11}}},
...
def test_영양_행은_집계_테이블_모양():
    ...
    assert [(r["date"], r["meal"]) for r in rows] == [("2026-08-24", "lunch"), ("2026-08-24", "dinner"), ("2026-08-25", "lunch")]
    r = rows[0]
    ...
    assert rows[2]["macro_ok"] is None and rows[2]["kgco2e"] is None


def test_석식_행은_dinner_절에서_나온다():
    d = nutrition_rows(MEAL, meals=("dinner",))
    assert [(r["date"], r["meal"]) for r in d] == [("2026-08-24", "dinner")]     # 석식 없는 날은 행이 안 나온다
    assert (d[0]["kcal"], d[0]["energy_pct"], d[0]["mar"], d[0]["kgco2e"]) == (802.0, 89, 88, 1.11)


def test_그날_메뉴는_정제된_이름():
    assert menu_on(MEAL, "2026-08-24") == ["현미밥", "김치찌개", "우유"]
    assert menu_on(MEAL, "2026-08-24", "dinner") == ["잡곡밥", "된장국"]
