...
def test_쓰기는_로컬만_바꾸고_템플릿은_그대로(env):
    ...
    doc["buffer_px"] = 25
    saved, bak = zones_store.write(doc, "t@example.com", local=local, now=dt.datetime(2026, 9, 3, 12, 0, 0))
    ...
    assert json.loads(local.read_text(encoding="utf-8"))["buffer_px"] == 25
...
def test_백업은_최신_5개만(env):
    ...
    for i in range(8):
        doc["buffer_px"] = i
        _, bak = zones_store.write(doc, "t", local=local, now=dt.datetime(2026, 9, 3, 12, 0, i))
...
def test_검증_실패는_아무것도_쓰지_않는다(env):
    ...
    with pytest.raises(zones_store.Invalid) as ei:
        zones_store.write(doc, "t", local=local)
    assert any("lambda_edge" in m for m in ei.value.errors) and not local.exists()
    with pytest.raises(zones_store.Invalid):
        zones_store.write(["not", "a", "doc"], "t", local=local)
    with pytest.raises(zones_store.Invalid):
        zones_store.write({**doc, "roi": None, "zones": []}, "t", local=local)
