...
| 대기시간 | Little's law: W = L / λ | L=ROI 점유 인원(**25초 중앙값 평활**, 09-17), λ=배식대 가상선 통과율(이동합, `.env RATE_WINDOW_SEC` 기본 180초 — 09-17 사용자 결정 2분→3분: 2분 창은 통과 1명=0.5명/분 계단이라 W 널뜀). **ROI 출구변 = λ 측정선**(같은 경계). λ < 0.5명/분이면 `insufficient_rate`. **공표 대기 = 90초 중앙값 평활**(09-17, 원시는 wait_raw_min 에 그대로) |
