...
- **중식/석식 분리(09-16 사용자 결정)**: insights.db 스키마 v2 — 전 표에 `meal` 열(lunch|dinner|all), rollup 은 `ROLLUP_WINDOW=lunch` 면 중식(`LUNCH_*`)·석식(`DINNER_*`, `.env`, 기본 17:00~18:30) 두 창을 각각 집계(all 모드는 한 행). meta 불변식은 '한 파일 = 한 창 모드·한 출처'로 완화, menu_stats·인기 기준값은 끼니별. **v1 파일은 백업 후 `--all` 재집계**.
  `/api/insight/*` 는 전부 `?meal=lunch|dinner`(기본 lunch) + 응답 `meal`. 화면: 히트맵은 중식·석식 2열(`.heat2`), 황금·예보·병목·품질·영양추이·TOP5 는 카드 오른쪽 위 끼니 토글(`core.mealSeg`, 기본 = 14시 전 중식·후 석식, 두 끼니를 함께 받아 캐시 — 전환에 재요청 없음). 석식 영양 지표·탄소는 `fetch_neis` 가 산출(**중식과 같은 '한 끼 권장량' 기준**, 09-16 확정) — 오늘급식 석식 열 아래 지표 한 줄, 탄소는 중식+석식 합산. 리포트(report.py)는 여전히 중식 결산(`meal IN ('lunch','all')`).
