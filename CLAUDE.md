...
- **PI_HOST는 Tailscale 주소**(.env 참조). 공용 체크아웃은 `/opt/mealboard`.
  **운영 구조(09-12 저녁 롤백 확정, 전 프로젝트 공통)**: 개발 메인은 **로컬 PC 작업 폴더**(이 저장소) — 다른 PC 에서는 Claude Code 원격제어로 로컬 PC 세션에 접속해 작업한다. 배포는 로컬 커밋 → `git push` → GitHub → Pi 에서 `git pull --ff-only`(또는 `deploy/deploy_pi.sh`). **Pi 에서 직접 코드 수정 금지, 작업트리는 항상 clean 유지** — pull 이 실패하면 Pi 쪽 오염부터 의심한다. (09-12 낮에 잠깐 도입했던 'Pi 워크트리 직접 편집·커밋' 방식은 같은 날 저녁 폐기.)
...
- **중식/석식 분리(09-16 사용자 결정)**: insights.db 스키마 v2 — 전 표에 `meal` 열(lunch|dinner|all), rollup 은 `ROLLUP_WINDOW=lunch` 면 중식(`LUNCH_*`)·석식(`DINNER_*`, `.env`, 기본 17:00~18:30) 두 창을 각각 집계(all 모드는 한 행). meta 불변식은 '한 파일 = 한 창 모드·한 출처'로 완화, menu_stats·인기 기준값은 끼니별. **v1 파일은 백업 후 `--all` 재집계**.
  `/api/insight/*` 는 전부 `?meal=lunch|dinner`(기본 lunch) + 응답 `meal`. 화면(09-18 개편): **대기시간 화면 카드(히트맵·황금·예보·병목·품질)는 토글 없이 현재 끼니만 자동 표시**(`defaultMeal`, 14시 전 중식·후 석식, 경계에서 `followMeal` 이 전체 전환 — 두 끼니를 함께 받아 캐시라 재요청 없음). 히트맵 2열(`.heat2`)·대기 화면 `.mealseg` 는 폐기, 끼니 토글(`core.mealSeg`)은 영양추이·TOP5 에만. 히트맵 '지금' 타일은 `data-now` CSS 점멸(30초 poll 의 `markHeatNow` 가 이동), 추이 막대는 호버·탭 툴팁 `#charttip`(chartGeo 역산). **히트맵 오늘 행 = 오늘 실황**(09-18 사용자 결정): `/api/insight/heatmap` 이 오늘 요일을 집계에서 빼고 queue.db 즉석 5분 빈(`_today_cells`)으로 채운다(`live_today`) — 지난주 같은 요일이 오늘 자리에 보이지 않게, 아직 안 온 시간대는 빈칸. 프론트는 히트맵을 30분 slow 에서 5분 `fastInsights` 로 옮겨 급식 중 타일이 차오른다. 리드 문구는 '셀을 누르면…' 만. 석식 영양 지표·탄소는 `fetch_neis` 가 산출(**중식과 같은 '한 끼 권장량' 기준**, 09-16 확정) — 오늘급식 석식 열 아래 지표 한 줄, 탄소는 중식+석식 합산. 리포트(report.py)는 여전히 중식 결산(`meal IN ('lunch','all')`).
...
- **배포 잔류 대책(09-11)**: 관리 UI 는 `?v=UI_VERSION`+`no-store`, 학생 화면은 새 서비스워커가 탭을 넘겨받을 때 자동 재로드 — 화면이 바뀌면 `core.UI_VERSION` 과 `sw.js CACHE` 를 같이 올린다(지금 v30).
...
- **자동 실측·보정(09-16 밤, B안 — 사용자 결정)**: `vision/counting.DwellTracker` 가 트랙의 **ROI 진입→λ선 통과 체류시간**(=실제 대기)을 재고, 보정계수 **K = median(실측 ÷ 진입 시점 예측)**(0.5~3.0, 이벤트 5개↑)로 **공표 대기 = 원시 W × K**. samples 에 `wait_raw_min`(원시)·`measured_wait_min`(체류 중앙값)·`calib`(K) 열 추가(ALTER, 옛 행 NULL — mock 은 NULL). 개인 체류값은 저장 안 함(창 요약 숫자만, §2 원칙).
  **판정(09-17 개정): '여유'는 절대 기준 30초 이하**(사용자 결정 — '여유' 보고 갔는데 밀려 있는 것보다 뜻밖의 여유가 낫다), '혼잡'은 그 창의 오늘 공표 대기 범위 상위⅓(`/api/status wait_range` — 30표본↑·폭 2분↑일 때만, 없으면 고정 12분 폴백), 나머지 보통. 히어로 각주에 "실측 보정 ×K · 오늘 범위". 한계: 가림으로 트랙이 끊기면 체류 과소(20초 미만 버림·중앙값으로 완충) — **수동 실측(A, 진입·도착 시각 기록)을 이따금 병행해 원시/보정/실측 세 값과 대조한다(사용자 합의)**.
- **다음 할 일**: ⓪ **대기시간이 실제보다 짧게 느껴진다(09-16 사용자 체감)**: B안 자동 보정 가동 — 실측 창에서 K·measured 가 실제와 맞는지 확인, 수동 실측과 대조. 기존 검증 = `served_est` vs 실제 식수 + 실사 HUD `L=` vs 눈 대조. 의심 순위: 화각 밖 줄 꼬리 > ROI 범위 > 가림 > λ선 왕복 +2(in 은 안 뺌). ① 월요일(9/14) 11:20 첫 실측 점심 — L·λ·평면도 마커 확인(마커가 밀리면 보정 y 4 m 조정), 14:10 첫 집계 뒤 히트맵 셀 확인 ② 안정되면 `queue.db.bak-*`(200 MB)·`.env.bak-*` 정리 ③ 석식을 집계 창에 넣을지·영양 지표 두 끼 기준 여부(영양 교사 판단) ④ Wide 카메라 오면 재보정 ⑤ **유출 키 교체(09-12)** — CF 터널 토큰·DeepL 키(사용자, 대시보드) ⑥ Inside Climate News 결이 다른 기사 섞임 — 교체 여부 관찰(후보 UNEP·Climate Home News) ⑦ 급식 있는 평일에 데스크톱 12컬럼 보드·모바일 dock 실물 확인, `/api/typical` 실측 후 확인.
...
| API 포트 | **8100** (.env `API_PORT`), 127.0.0.1 바인딩. 관리 앱 **8101**(`ADMIN_PORT`), vision 디버그 MJPEG **8102**(`DEBUG_PORT`), 메타데이터 소켓 `/run/mealboard/meta.sock` — 전부 127.0.0.1. hailo-ollama(수동 시작) **8000** 127.0.0.1 | 8501·1883 은 Plant(8000 은 09-04 사용자 확인으로 hailo-ollama 에). **공개 주소 `kjhs-meal.com` 은 8100 만**. 관리 앱은 `tailscale serve --https=8443`(tailnet) 과 **`admin.kjhs-meal.com`(같은 터널, 앞단 Cloudflare Access 이메일 코드 로그인, 09-11 사용자 결정)** 두 길 — Access 없이 공개 호스트에 8101 매핑은 금지. Cloudflare 를 거쳐 온 요청은 Access JWT 검증(`app/admin/access.py`)만 신원으로 친다(`.env CF_ACCESS_TEAM`·`CF_ACCESS_AUD`, 비면 닫힘). `serve reset`·`funnel reset` 금지(Funnel 443 까지 지워진다) |
...
| 대기시간 | Little's law: W = L / λ | L=**화면 안 트랙 전체**(09-18 사용자 결정, `.env QUEUE_SOURCE=tracks` 기본 — ROI 가 실제 줄보다 좁아 '사람은 많은데 1명' 이 되던 문제; 앉은 인원이 많이 잡히면 `roi` 로 회귀)(**25초 중앙값 평활**, 09-17), λ=배식대 가상선 통과율(이동합, `.env RATE_WINDOW_SEC` 기본 180초 — 09-17 사용자 결정 2분→3분: 2분 창은 통과 1명=0.5명/분 계단이라 W 널뜀). **λ 측정선 = ROI 지정 변(lambda_edge)에서 안쪽으로 평행 생성**(09-21 사용자 설계: `roi.lambda_inset`(정규화, 편집기 기본 0.04)·`lambda_scale`(길이 배율) — `vision/zones.lambda_line`, 편집기 lamLine 과 동일 기하. **λ선을 화면 경계에 붙이면 부호 변화가 불가능해 λ 가 영구 0**(09-21 사고) — inset 이 이를 구조적으로 막는다. ROI 폴리곤은 화면 바닥까지 붙여도 된다. 발이 프레임 하단에 잘린 트랙은 counter 가 측정선 안쪽으로 투영해 멤버십 판정). λ < 0.5명/분이면 `insufficient_rate`. **공표 대기 = 90초 중앙값 평활 + 하한**(09-17): L≥1 인데 원시가 실측 체류 중앙값(measured_wait_min)보다 낮으면 체류 중앙값으로 올린다(수동 실측 12:44=1.3분 vs 원시 0분 대조에서 확정 — L 누락 오차는 곱셈 K 로 못 살린다). 원시는 wait_raw_min 에 그대로. 체류 실측엔 `.env DWELL_BIAS`(기본 1.0, Pi 는 09-17 수동 실측 대조로 1.4 — 트랙 끊김 과소 편향, 실측 쌓이면 조정) |
...
- 코드 반영: `/opt/mealboard` 에서 `git pull --ff-only`(또는 `bash deploy/deploy_pi.sh`) 후 해당 서비스만 `sudo systemctl restart mealboard-api`(vision·mock은 아래 주의 참조). 의존성이 바뀌면 `uv sync`(**vision 은 반드시 `uv sync --extra vision`**), 유닛이 바뀌면 `bash setup_pi.sh`
...
- `/opt/mealboard` 안에서 파일 직접 편집 — 코드는 로컬 PC 에서 커밋해 pull 한다(09-12 저녁 롤백 확정, §0 운영 구조)
...
- push는 매 작업 세션 종료 시. 사용자가 요청하면 중간에도. Pi는 자동으로 pull하지 않는다 — 배포는 사람(또는 Claude)이 명시적으로 `git pull --ff-only`/`deploy/deploy_pi.sh` 로
